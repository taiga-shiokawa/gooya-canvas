...
- **改行コードは `.gitattributes` の `* text=auto eol=lf` で LF に統一する**（対応済み）。Prettier は改行を LF に揃えるため、これが無いと Windows（`core.autocrlf=true`）で clone した直後に `npm run format:check` が全ファイルで落ちていた。バイナリ拡張子は変換対象から除外する。
